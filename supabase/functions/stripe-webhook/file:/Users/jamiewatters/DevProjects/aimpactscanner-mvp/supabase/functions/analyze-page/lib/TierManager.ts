...
export class TierManager {
   ...
   */ async validateAnalysisAccess(userId) {
    try {
      ...
      try {
        const { data: user, error: userError } = await this.supabase.from('users').select('tier, tier_expires_at, monthly_analyses_used, monthly_reset_date').eq('id', userId).single();
        if (userError || !user) {
   ...
   */ async recordUsage(usage) {
    try {
      ...
      if (usage.tier === 'free') {
        try {
          await this.supabase.rpc('increment_monthly_analyses', {
      ...
      try {
        const { error: analyticsError } = await this.supabase.from('usage_analytics').insert({
   ...
   */ async upgradeToTier(userId, tier, subscriptionData) {
    try {
      console.log(`🚀 Upgrading user to ${tier} tier:`, userId);
      const expiresAt = new Date();
      expiresAt.setMonth(expiresAt.getMonth() + 1); // 1 month from now
      // Update user tier
      const { error: userError } = await this.supabase.from('users').update({
        tier: tier,
      ...
      const { error: subError } = await this.supabase.from('subscriptions').upsert({
        user_id: userId,
        tier: tier,
   ...
   */ async downgradeTier(userId, reason = 'subscription_ended') {
    try {
      ...
      const { error: subError } = await this.supabase.from('subscriptions').update({
        status: 'canceled',
   ...
   */ async getUserUsageAnalytics(userId, days = 30) {
    try {
      const fromDate = new Date();
      fromDate.setDate(fromDate.getDate() - days);
      const { data, error } = await this.supabase.from('usage_analytics').select('*').eq('user_id', userId).gte('created_at', fromDate.toISOString()).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
   ...
   */ static getAvailableUpgrades(currentTier) {
    const upgradeMap = {
      'free': [
        'coffee',
        'professional',
