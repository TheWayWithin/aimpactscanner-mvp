 ...
 *
 * Creates 6 products (Solo, Growth, Scale x monthly/annual) in Stripe Test Mode
...
// Test mode API key from .mcp.json
const STRIPE_API_KEY = 'REMOVED_STRIPE_TEST';
...
async function main() {
  console.log('🚀 Starting Stripe Product Setup (Test Mode)\n');
  console.log('This will create 3 products with 6 prices total:\n');
  ...
  try {
    ...
    console.log('═══════════════════════════════════════════════════════\n');
    console.log('📋 PRICE IDS (for STRIPE-PRICE-IDS.md):\n');
    ...
    const fs = require('fs');
    fs.writeFileSync('stripe-setup-results.json', JSON.stringify(results, null, 2));
    console.log('💾 Results saved to: stripe-setup-results.json\n');

    console.log('📝 Next steps:');
    console.log('1. Copy the Price IDs above to STRIPE-PRICE-IDS.md');
    console.log('2. Verify products in Stripe Dashboard: https://dashboard.stripe.com/test/products');
    console.log('3. Test checkout sessions for each price');
    console.log('4. Verify Growth Annual shows trial period in Stripe Dashboard\n');
